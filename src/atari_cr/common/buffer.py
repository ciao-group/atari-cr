...
class BaseBuffer(ABC):
    ...
    @abstractmethod
    def _get_samples(
        self, batch_inds: np.ndarray, env: Optional[VectorEnv] = None
    ...
    @staticmethod
    def _normalize_reward(reward: np.ndarray, env: Optional[VectorEnv] = None) -> np.ndarray:
        if env is not None:
...
class ReplayBuffer(BaseBuffer):
    ...
    def sample(self, batch_size: int, env: Optional[VectorEnv] = None) -> ReplayBufferSamples:
        """
...
class RolloutBuffer(BaseBuffer):
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VectorEnv] = None) -> RolloutBufferSamples:
        data = (
...
class DictReplayBuffer(ReplayBuffer):
    ...
    def sample(self, batch_size: int, env: Optional[VectorEnv] = None) -> DictReplayBufferSamples:
        """
        Sample elements from the replay buffer.
        :param batch_size: Number of element to sample
        :param env: associated gym VecEnv
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VectorEnv] = None) -> DictReplayBufferSamples:
        # Sample randomly the env idx
        env_indices = np.random.randint(0, high=self.n_envs, size=(len(batch_inds),))
...
class DictRolloutBuffer(RolloutBuffer):
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VectorEnv] = None) -> DictRolloutBufferSamples:
