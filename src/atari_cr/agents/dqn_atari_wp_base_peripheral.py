...
def parse_args():
    ...
    # fov setting
    parser.add_argument("--fov-size", type=int, default=0)
    ...
    # pvm setting
    parser.add_argument("--pvm-stack", type=int, default=1)
                ...
                for eval_ep in range(args.eval_num):
                    ...
                    while not done:
                        ...
                        next_obs, rewards, dones, _, infos = eval_env.step({"motor_action": motor_actions, 
                                                                            "sensory_action": [sensory_action_set[fov_idx]]})
                        obs = next_obs
                        fov_idx = (fov_idx + 1) % len(sensory_action_set)
                        done = dones[0]
                        if done:
                            ...
                            if args.capture_video:
                                ...
                                record_file_fn = f"seed{args.seed}_step{global_transitions:07d}_record.pt"
