...
FovType: TypeAlias = Literal["window", "exponential"]

class Fovea():
    def __init__(self, type: FovType, size: tuple[int, int]):
        self.type = type
        self.size = np.array(size, dtype=np.int32)

    def apply(self, img: np.ndarray, fixations: list[tuple[int, int]]):
        """ Applies the fovea to an image """
        match self.type:
            case "window":
                raise NotImplementedError()
            case "exponential":
                return foveat_img(img, fixations)
