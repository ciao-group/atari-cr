...
class DatasetWithHeatmap:
    ...
    def createGazeHeatmap(self, gaze_coords: List[List[float]], heatmap_shape, viz=False, asc=False, asc_file=''):
    ...
    def get_gaze_data(self, gaze_coords: List[List[float]]):
        frameid2pos = {}
        frame_id = 0
    ...
    def preprocess_gaze_heatmap(self, bg_prob_density: float, debug_plot_result=False):
        ...
        if self.sigmaH > 1 and self.sigmaW > 1:  # was 0,0; if too small, dont blur
            ...
            model.add(K.layers.Conv2D(1, kernel_size=gkernel.shape, strides=1, padding="valid", use_bias=False,
                                      activation="linear", kernel_initializer=K.initializers.Constant(gkernel[:, :, np.newaxis, np.newaxis])))
        else:
        ...
        output = model.predict(self.GHmap, batch_size=512)
...
if __name__ == "__main__":
    with open("52_gazes.pkl", "rb") as f: gazes = pickle.load(f)
    saliency_maps = DatasetWithHeatmap().createGazeHeatmap(gazes, 84)
    with open("52_saliency.np", "wb") as f: np.save(f, saliency_maps)